...
/** 用户/群数据写入口：备份轮转 + 原子写（原子写见 lib/jsonFile.js，indent 2 为本文件既有格式） */
function writeJson(file, data) {
  backupFile(file)
  writeJsonAtomic(file, data, { indent: 2 })
}
...
/**
 * 批量写群设置（`#mai push global` 一次给全部群置位）。
 * 逐群调用 updateGroup 会做 N 次「整库序列化 + 备份拷贝 + 落盘」，群多时是 O(N²)，故合并成一次写。
 * @param {Record<string|number, {guess?: boolean, aliasPush?: boolean}>} patches
 * @returns {number} 写入的群数
 */
export function updateGroups(patches) {
  const file = path.join(dataRoot, 'group.json')
  const disk = readJson(file, {})
  const merged = { ...groupDb, ...disk }
  ...
  for (const groupId of keys) {
    const key = String(groupId)
    merged[key] = { guess: false, aliasPush: false, ...(disk[key] || groupDb[key] || {}), ...patches[groupId] }
  }
  groupDb = merged
  writeJson(file, merged)
  return keys.length
...
/**
 * 别名推送白名单（只有显式 `#mai push on` 过的群才推，D4 刻意偏离源的黑名单）
 * 供 `lib/aliasSse.js` 取广播目标——**不需要**枚举 bot 的全部群，
 * 这也是白名单化带来的简化（源要「遍历全部群再减 disable 列表」）。
 * @returns {string[]} 群号（`group.json` 的键，字符串形式；`Bot.pickGroup` 自己会转数字）
 */
export function groupsWithAliasPush() {
  return Object.entries(groupDb)
    .filter(([, row]) => row?.aliasPush === true)
    .map(([gid]) => gid)
}
