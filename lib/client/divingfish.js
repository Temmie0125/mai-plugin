...
export class DivingFishAPI extends ApiClient {
  ...
  async _handleError(resp) {
    ...
    if (resp.status === 403) {
      /**
       * 403 有两个来源，用户可见文案完全不同，按端点区分（源一律按 oauth 与否二分，会把前者误报成「尚未授权」）：
       * ・`/query/*` 公开查询（不带任何凭据，见 queryUserB50）：目标用户没开「允许他人查询」→ 403。
       *   oauth 用户的 B50 走的正是这条路，源会告诉一个明明绑定好好的用户「您尚未授权本 BOT」。
       * ・`/player/*` 代用户端点（带 Bearer）：令牌没覆盖该权限（授权被撤销或范围不足）→ 确实该报未授权。
       */
      const url = String(resp.url || '')
      let detail = ''
      try { detail = (await resp.text()).slice(0, 120) } catch { /* 无响应体 */ }
      logger.warn(`[mai-plugin] 水鱼 403：${url || '(未知 URL)'}（oauth=${this.oauth}）${detail ? `｜${detail}` : ''}`)
      if (this.oauth && url.includes('/player/')) throw new DivingFishNotAuthorizedError()
  ...
  /**
   * 用户指定版本的牌子成绩（源 client.py:116 query_user_plate）
   * oauth → POST /player/plate（独立 payload，不携带 qq）；否则 → POST /query/plate
   * @param {string[]} version 版本名列表（源 VERSION_MAP 的版本串）
   * @returns {Promise<Array>} verlist 原始条目（PlayInfoDefault 形状）
   */
  async queryUserPlate(version) {
    let result
    if (this.oauth) {
      // ⚠️ 与 queryUserRecord 同理：oauth 分支必须独立 payload，否则会把 qq 泄漏进 /player/plate
      result = await this._requestOAuth('POST', '/player/plate', { json: { version } })
    } else {
      this.json.version = version
      result = await this._request('POST', '/query/plate', { json: this.json })
    }
    return result?.verlist ?? []
  }

  /**
   * 用户全部成绩（源 client.py:135 query_user_records）
   * oauth → GET /player/records；否则 → GET /dev/player/records（分页载荷走 params）
   * @returns {Promise<Array>} records 原始条目（PlayInfoDev 形状）
   */
  async queryUserRecords() {
    const result = this.oauth
      ? await this._requestOAuth('GET', '/player/records')
      : await this._request('GET', '/dev/player/records', { params: this.json })
