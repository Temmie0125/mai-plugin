...
async function renderB50Like({ user, player, best50, variantLabel = null, fit = false, fitTotal = 0 }) {
  const view = b50View({
...
/**
 * `#mai 歌50 [难度色]<歌曲>`：整张 B50 全是同一首歌（该谱重复填充 B35+B15，V1）
 *
 * @param {object} song Song
 * @param {number|null} levelIndex 未指定（null）时取该曲**玩家已有成绩里 rating 最高**的那一条的难度
 *   （需求原文「不指定则取该歌曲的最高 Rating」；一条成绩都没有 → 回「未找到该曲目的成绩」）
 */
export async function drawSong50(user, song, levelIndex = null) {
  return handleErrors(async () => {
    const records = await getPlayerResultCached(user)
    if (records?.dfHint) return records.dfHint
    const list = records ?? []
    ...
    const spec = repeatSpec({ song_id: song.song_id, level_index: li }, `歌50 · ${song.song_name}`)
