...
export async function drawBest50(user, { username = null, allPerfect = false } = {}) {
  return handleErrors(async () => {
    // 水鱼无 AP50 接口 ⇒ 由本地全量成绩算（设计 §5 / V4）；落雪侧仍有 /bests/ap，走下面的变体流。
    ...
    if (allPerfect && !username && effectiveService(user) !== 'lxns') {
      return await drawAp50Local(user)
    }
...
/**
 * 水鱼 AP50：水鱼无 AP50 接口 ⇒ 由全量成绩本地算（设计 §5，V4）
 *
 * 数据取每日本地缓存（与 table/plate/拟合b50 同口径，`#mai update` 可强制刷新）；
 * 头部仍实时。落雪侧不走这里（有 `/bests/ap` 接口，见 getBest50）。
 */
export async function drawAp50Local(user) {
  const head = await getPlayerHeader(user)
  if (head.dfHint) return head.dfHint
  const records = await getPlayerResultCached(user)
  if (records?.dfHint) return records.dfHint
  ...
  if (candidates === 0) return '未找到 AP 成绩。'
  return await renderB50Like({ user, player: head.player, best50, variantLabel: AP_SPEC.label })
}
