...
export function normalizeRank(raw) {
  ...
  if (!t) return null
  if (Object.hasOwn(RANK_CN, t)) return RANK_CN[t]
 ...
 * 该评级的起算线：d 档无起算线（记 0）
 *
 * 注意这是**下界**：SS+ = 99.5（不是 100）、SSS = 100（不是 100.5）——上界是 `ACHIEVEMENT_LIST[i]`。
 * `lib/simScore.js` 的「指定评级 → 模拟达成率」也复用此函数，改口径会同时改两处行为。
 */
export const rankStart = key => {
 ...
 *
 * ⚠️ 用户文本查表一律 `Object.hasOwn`，不可裸取 `MAP[t]` 或 `t in MAP`：
 * `'toString'` / `'constructor'` / `'__proto__'` 会命中原型链上的现成值（且真值），
 * 于是「不是词表里的词」会被判成「是」，用户拿到的是静默走错分支的结果。
 */
export function isTimeKeyword(token) {
  return Object.hasOwn(TIME_ALIAS, String(token ?? '').trim())
}
...
export function listPresetOf(token) {
  ...
  if (Object.hasOwn(LIST_KEY_ALIAS, t)) return LIST_KEY_ALIAS[t]
  if (Object.hasOwn(LIST_KEY_ALIAS, lower)) return LIST_KEY_ALIAS[lower]
  return null
}
...
const HELP_ROWS = [
  ...
  {
    group: '模拟', kind: 'song50', usage: '歌50 [难度色]<曲名|id|别名> [模拟参数…]',
    eg: '#{head} 歌50 白潘 理论 FDX+ 5星 AP+',
    tokens: ['歌50 紫茄子', '歌50 799 红', '歌50 白潘 理论 FDX+ 5星 AP+'],
    desc: '整张 B50 全是同一首歌（该谱重复填充 B35+B15，合计 = 50 × 该谱 Rating）。'
      + '追加达成率（100.1145% / 99.00）或评级（理论 / SS+ / 鸟+）即为模拟：不再看你的实际成绩，'
      + '缺省取该曲定数最高的谱面。其余参数顺序随意 —— 同步 FDX+/FDX/FS+/FS/SYNC/单刷、'
      + 'DX dx1145 或 dx99% 或 N星、标志 FC/FC+/AP/AP+。横幅上的参数可原样复制回来当命令',
  },
]
 ...
 * @param {object|null} [record] 预置的成绩（**模拟模式**，见 lib/simScore.js）。
 *   给了它就**完全不走** records 过滤——`variantBest50` 直接拿它填 35/15 份。
 *   不给则按老路从传入的 records 里挑该谱 rating 最高的一条。
 */
export function repeatSpec(chart, label, record = null) {
  return {
    key: 'song50', label: label || '歌', mode: 'repeat',
    songId: chart.song_id, levelIndex: chart.level_index, record,
  }
}
...
export function resolveDesigner(raw, { designers, alias = designerAlias } = {}) {
  const t = String(raw ?? '').trim()
  if (!t) return null
  // 别名表键大小写不敏感（表里有 latin 键 happy，用户多半会打成 Happy）；
  // 查表同样走 Object.hasOwn，理由见 isTimeKeyword
  const lower = t.toLowerCase()
  let mapped = t
  if (Object.hasOwn(alias, t)) mapped = alias[t]
  else if (Object.hasOwn(alias, lower)) mapped = alias[lower]
