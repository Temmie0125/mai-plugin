...
import { collectionSrc, songChartFile } from './assets.js'
...
import { qqLogoUrl } from '../client/http.js'
...
/** 缺 `music_icon_dxstar_*.png` 的告警只打一次（一页要取 5 张，逐张刷屏没意义） */
let starIconWarned = false

/**
 * 分数线海报的 DX 星徽章（星形底 + 白色数字，label 为 '5'..'1'）
 *
 * 样板用的是 `Resources/music_icon_dxstar_N.png`，本仓静态包里**暂无**这套图
 * （既有 `status_dxstar_N.png` 是圆角标签、`UI_GAM_Gauge_DXScoreIcon_0N.png` 是五颗小星，形状都不同）。
 * 故优先取新图、缺图时回退到既有图标并告警一次，等资源仓库补图后自动切换。
 */
export function dxStarIcon(label) {
  const wanted = `music_icon_dxstar_${label}.png`
  if (fs.existsSync(path.join(staticRoot, 'mai', 'pic', wanted))) return picSrc(wanted)
  if (!starIconWarned) {
    starIconWarned = true
    const logger = global.logger || console
    logger.warn?.(
      `[mai-plugin] 静态资源缺少 ${wanted}（分数线海报的 DX 星徽章），` +
        '已回退到 status_dxstar_*.png。补齐后自动生效（见 README「安装与资源」）',
    )
  }
  return picSrc(`status_dxstar_${label}.png`)
}
 ...
 * @param {boolean} [opts.fit] 拟合模式：定数行显示**拟合定数**（2 位小数，取 info.level_value）
 */
export function whiledraw(images, texts, data, { dx = false, listY = 0, theme = 'prism_plus', fit = false } = {}) {
  const gapY = 114
  const dxStep = 276
  const startX = 16
  ...
  data.forEach((info, num) => {
    ...
    // 拟合模式：定数行显示拟合定数——fitBest50 已把它覆写进 info.level_value，
    // 且用 pyRound2（2 位小数）以区别于真实定数的 pyFloat（13.0 这种整值带 .0）
    const dsLine = fit
      ? `${pyRound2(info.level_value)} -> ${info.rating}`
      : `${pyFloat(mai.totalLevelValueMap[`${info.song_id}-${li}`])} -> ${info.rating}`
    texts.push(txt(dsLine, x + 93, y + 65, 15, { font: 'tb', color: DIFF_TEXT_COLOR[li], anchor: 'lm' }))
  })
}
 ...
 * @param {object} opts.player Player（**真实值**：头部 DX Rating 数字图照常由它驱动）
 * @param {object} opts.best50 Best50（fit/variant 模式下其 sd/dx 为条件口径、totals 为条件合计）
 * @param {string} opts.serviceName Data from xxx
 * @param {string} opts.botName
 * @param {boolean} [opts.fit] 拟合模式：定数行改显拟合定数、称号位改显拟合合计行
 * @param {number} [opts.fitTotal] 拟合总分（fit 模式的合计行末端）
 * @param {string|null} [opts.variantLabel] 随心配变体名（如 'FC' / '东方' / 'はっぴー'）。
 *   传了就走「变体模式」：称号位改显**条件合计**（右端为 x+y，不再是玩家真实 Rating，见 V9），
 *   变体名画在称号框**下方横幅**（见下方 banner 注释）。
 * @param {string|null} [opts.variantLabelTail] 横幅尾段（歌50 模拟参数，如 '· 模拟 101.0000% FDX+ 5★ AP+'）。
 *   与 variantLabel 的区别：**长曲名顶掉的是前缀，不是它**——超宽时只截断 variantLabel，
 *   尾段永远完整显示（模拟参数是这条命令唯一的信息量，被 '...' 吃掉就等于没标）。
 */
export function b50View({ theme = 'prism_plus', qqid = null, player, best50, serviceName, botName, fit = false, fitTotal = 0, variantLabel = null, variantLabelTail = null }) {
  const images = []
  const texts = []
  ...
  // plate
  // 收藏品（id 形态）：资源包 mai/plate/<id>.webp 优先，缺图才在线取并缓存；
  // 在线回退若被 assetsOnline 关掉则 collectionSrc 返回空串，就地回退默认牌
  let plateSrc
  if (!player.name_plate) {
    plateSrc = picSrc('UI_Plate_550101.png')
  } else if (typeof player.name_plate === 'object') {
    plateSrc = collectionSrc('plate', player.name_plate.id) || picSrc('UI_Plate_550101.png')
  } else {
  ...
  // icon（收藏品 id 形态同上走本地优先；无收藏品时才是 QQ 头像，那条链路不属「切图回退」）
  let iconSrc
  if (player.icon) {
    iconSrc = collectionSrc('icon', player.icon.id) || picSrc('UI_Icon_509506.png')
  } else if (qqid) {
  ...
  let shougouColor
  if (fit) {
    // 拟合模式：称号位固定展示**拟合合计行**，不展示真实称号——拟合信息是该图的主体
    // （末端的 fitTotal 与头部真实 DX Rating 数字图刻意不同源，见设计 D1）
    trophyTitle = `拟合 B35: ${best50.sd_total} + B15: ${best50.dx_total} = ${fitTotal}`
    trophyFont = 'tb'
    shougouColor = 'Rainbow'
  } else if (variantLabel) {
    // 变体模式（随心配/水鱼AP50）：称号位固定展示**条件合计**，不展示真实称号。
    // ⚠️ 右端是 sd_total + dx_total，**不是** player.rating —— 变体的合计与真实 Rating 不同源（V9）。
    // 变体名不写在这里：称号框只有 270px 宽，真实量级下 `B35: 10114 + B15: 4305 = 14419`（31 列）
    // 已把它占满，塞前缀会把数字挤出框外（拟合模式的「拟合 」前缀是唯一还塞得下的特例）。
    trophyTitle = `B35: ${best50.sd_total} + B15: ${best50.dx_total} = ${best50.sd_total + best50.dx_total}`
    trophyFont = 'tb'
    shougouColor = 'Rainbow'
  } else if (player.trophy) {
  ...
  // 变体名横幅（称号框下缘 187 → 成绩格起点 235 之间的整条留白，画布 1400 宽，不受称号框窄条限制）
  // 用「黑字 + 白描边」：与称号框正文同色系，且在两个主题的浅色背景上都读得清
  if (variantLabel) {
    // 列的度量是**半角宽度**（textwidth.coloumWidth：ASCII=1、CJK=2），22px 字号下 1 列 ≈ 11px，
    // 居中画在 700 ⇒ 画布单侧 700px 可用 ≈ 127 列。取 110 列（≈1210px）留出描边与边距。
    // （初版兜底值是 60 列 ≈ 660px，不到画布一半；长曲名会先被截断，正是 §7.1 实施记录里说的
    //  「整条留白够长」被浪费掉的情形。）
    const BANNER_COLS = 110
    // 尾段（歌50 的模拟参数）**必须完整可见**，先从预算里扣掉；余量才给可截断的前缀。
    const tail = variantLabelTail ? ` ${variantLabelTail}` : ''
    const body = truncateByWidth(variantLabel, Math.max(12, BANNER_COLS - coloumWidth(tail)))
    texts.push(txt(body + tail, 700, 213, 22,
      { font: 'tb', color: BLACK, anchor: 'mm', stroke: 3, strokeColor: WHITE }))
  }
  ...
  // 成绩格（源 whiledraw）
  whiledraw(images, texts, best50.sd, { dx: false, theme, fit })
  whiledraw(images, texts, best50.dx, { dx: true, theme, fit })
...
/**
 * 别名投票列表（源 `mai_alias.py:125-155` 是长文本→图；设计 §6.4 废除转图后此处仍出图，
 * 因为投票行**含曲绘与版本图标**，图比转发列表可读得多）
 *
 * 版式复用曲目列表（同模板 songlist.html、同卡片坐标），差异只在信息区：
 *   曲师 → 待投别名　BPM → 票数　所属分区 → 曲目 ID　难度值 → 投票编号 + 标签
 *
 * `#N` 是**跨页连续**的全局序号（第 2 页从 15 起），与 `apps/song.js` 的
 * `#mai alias vote #N` 一一对应——两者都基于「同一份列表按 song_id、tag 排序」这一口径，
 * 见 `sortVotes()`。
 */
export function voteListView({ votes, page, totalPage, botName, startIndex = 0 }) {
  const images = []
  const texts = []

  const lines = Math.floor(votes.length / 2) + (votes.length % 2)
  const height = 200 + lines * 145 + 200
  ...
  votes.forEach((vote, num) => {
    ...
    const x = startX + col * xGap
    const y = startY + row * yGap
    // 投票只带 song_id；曲绘/版本图标/类型图标都取自曲库（未收录的新曲则只留封面位）
    const song = mai.totalList.byId(vote.song_id)

    images.push(img(picSrc('song_card.png'), x, y, null, null, { z: 2 }))
    images.push(img(songChartSrc(vote.song_id), x + 10, y + 10, 80, 80, { z: 2 }))
    if (song) {
      images.push(img(picSrc(`${song.version_str}.png`), x + 315, y - 30, 104, 50, { z: 2 }))
      images.push(img(picSrc(`${song.type.toUpperCase()}.png`), x + 50, y + 75, 40, 15, { z: 2 }))
      // ⚠️ 刻意**不画** sl_diff.png 难度彩条：投票没有难度维度，彩条纯属误导，
      //    且它会把下方的编号/标签压成低对比（白字盖浅色条）。详见 voteListView 头注。
    }

    // 编号：与投票用的 `#N` 同一序号，与标签同排紧邻（「#1 对应某 tag」一眼可读）。
    // 无彩条垫底 ⇒ 用卡片正文色而非白色（白字在浅色毛玻璃卡上不可读）
    texts.push(txt(`#${startIndex + num + 1}`, x + 50, y + 103, 19, { font: 'fot', color: DEFAULT_TEXT_COLOR, anchor: 'mm' }))
    const tag = truncateByWidth(vote.tag, 13)
    texts.push(txt(tag, x + 100, y + 103, 15, { font: 'fot', color: DEFAULT_TEXT_COLOR, anchor: 'lm' }))

    const title = vote.name || song?.song_name || `ID ${vote.song_id}`
    texts.push(txt(truncateByWidth(title, 20), x + 100, y + 25, 20, { font: 'fot', color: DEFAULT_TEXT_COLOR, anchor: 'lm' }))
    texts.push(txt(`待投别名 ${truncateByWidth(vote.apply_alias, 24)}`, x + 100, y + 50, 12, { font: 'fot', color: DEFAULT_TEXT_COLOR, anchor: 'lm' }))
    texts.push(txt(`票数 ${vote.agree_votes ?? 0}/${vote.votes ?? 0}`, x + 100, y + 80, 15, { font: 'fot', color: DEFAULT_TEXT_COLOR, anchor: 'lm' }))
    texts.push(txt(`ID ${vote.song_id}`, x + 230, y + 80, 12, { font: 'sy', color: DEFAULT_TEXT_COLOR, anchor: 'lm' }))
  })

  texts.push(txt('别名投票', 500, 70, 55, { font: 'fot', color: DEFAULT_TEXT_COLOR, anchor: 'mm', stroke: 3, strokeColor: WHITE }))
  texts.push(txt(`Page ${page}/${totalPage}`, 500, height - 100, 35, { font: 'fot', color: DEFAULT_TEXT_COLOR, anchor: 'mm', stroke: 3, strokeColor: WHITE }))
  texts.push(txt(
    `Designed by Yuri-YuzuChaN & BlueDeer233. Generated by ${botName} BOT`,
    500, height - 30, 18, { font: 'fot', color: DEFAULT_TEXT_COLOR, anchor: 'mm', stroke: 3, strokeColor: WHITE }
  ...
  return { width: 1000, height, bgCss: tricolorGradientCss(), card, images, texts }
}

/** 按列宽截断（与曲目列表同款 coloumWidth/changeColumnWidth 口径） */
export function truncateByWidth(text, max) {
  const s = String(text ?? '')
  return coloumWidth(s) > max ? changeColumnWidth(s, max - 1) + '...' : s
}
