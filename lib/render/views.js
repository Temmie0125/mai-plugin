 ...
 * anchor 对应 PIL 文字锚点：lt / lm / rm / mm。
 */
...
/** 源 song_chart：id % 10000 曲绘，缺图回退 0.png */
export function songChartSrc(songId) {
  const id = Number(songId) % 10000
  const p = path.join(staticRoot, 'mai', 'cover', `${id}.png`)
  if (fs.existsSync(p)) return `file:///${p.replace(/\\/g, '/').replace(/ /g, '%20')}`
  const fallback = path.join(staticRoot, 'mai', 'cover', '0.png')
  return `file:///${fallback.replace(/\\/g, '/').replace(/ /g, '%20')}`
}
...
const DIFF_BG_NAMES = ['basic', 'advanced', 'expert', 'master', 'remaster']

/** 图片元素（w/h 为 null 时按原始尺寸显示；z 默认 1，模板不再兜底） */
function img(src, x, y, w = null, h = null, extra = {}) {
  return { src, x, y, w, h, z: 1, ...extra }
}

/** 文字元素（z 默认 3，模板不再兜底） */
function txt(text, x, y, size, { font = 'sy', color = WHITE, anchor = 'lt', stroke = 0, strokeColor = WHITE } = {}) {
  ...
  return { text: String(text), x, y, size, font, color, anchor, stroke, z: 3, shadow }
}
...
export function b50View({ theme = 'prism_plus', qqid = null, player, best50, serviceName, botName }) {
  ...
  // 成绩格（源 whiledraw）
  const whiledraw = (data, dx, listY = 0) => {
    const gapY = 114
    const dxStep = 276
    const startX = 16
  ...
  whiledraw(best50.sd, false)
  whiledraw(best50.dx, true)
