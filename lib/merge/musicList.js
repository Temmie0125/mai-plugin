...
export class MusicList {
   ...
   * @param {object} opts 见源签名；level_value/bpm/notes 可为 [min,max] 二元组
  ...
  filter(opts = {}) {
    ...
    let { level_value, bpm, notes } = opts
    ...
    for (const song of this.root) {
      ...
      for (const diff of song.difficulties) {
        ...
        // 物量（谱面 note 总数；本仓扩展，源无此过滤）——按难度逐谱比对
        if (notes != null) {
          if (Array.isArray(notes)) {
            if (!(notes[0] <= diff.notes.total && diff.notes.total <= notes[1])) continue
          } else if (diff.notes.total !== notes) continue
        }
