 ...
 * 另有两处**落雪独有曲的口径归一**（水鱼曲库滞后时才有落雪独有曲，两个查分器字段口径不同）：
 *   ① `genre`：落雪内部串 → 水鱼口径中文分类名（`LXNS_GENRE_CN`）
 *   ② `isnew`：按**大版本块**比对（落雪 difficulty.version 是子版本，如 25504 属于 25500）
 * 两处都在 `setVersion` 的建曲分支里，注释详见该处。
 */
import { DX_CN_VERSION, LXNS_GENRE_CN } from '../constants.js'
  ...
  if (lxnsList != null) {
    ...
    const setVersion = (raw, verType, sid, diffs) => {
      ...
      song = Song({
        ...
        song_name: raw.title,
        artist: raw.artist,
        // 落雪的 genre 是内部串（maimai / オンゲキCHUNITHM …），水鱼与合并库用中文分类名
        // （舞萌 / 音击&中二节奏 …）⇒ 归一；表内未收录的串原样保留（见 LXNS_GENRE_CN）
        genre: LXNS_GENRE_CN[raw.genre] ?? raw.genre,
        bpm: raw.bpm,
        version_str: DX_CN_VERSION[verMap[diffVer]]?.[1] ?? '',
        version_int: base.version,
        type: verType,
        // ⚠️ 双侧都取**大版本块**：落雪的 `difficulty.version` 是子版本（25504），而
        //    `versions[].version` 是大版本（25500）。源侧按 `newVersion === base.version`
        //    精确比对，于是「水鱼滞后、落雪独有」的当期新曲全被判进旧版池 B35
        //    （实测 2026-09-16：10 个条目，例 11820 Xaleid◆scopiX，version 25504 / 当期 25500）。
        //    diffVer 就是上面已归到块的值（version_str 查表同用），口径保持一致。
        isnew: newVersion === diffVer,
        difficulties,
      })
...
export function mergeAliasData({ yuzuAliases, lxnsAliases = null, localAliasData = null }) {
  ...
  if (lxnsAliases != null) {
    for (const item of lxnsAliases.aliases) {
      ...
      // 落雪 DX 谱面 id 需要 +10000 映射；宴谱 id ≥ 100000 不能再偏移（源 bc19b45 修复）
      if (songId > 1000 && songId < 100000) songId += 10000
