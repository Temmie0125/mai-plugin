...
import { Alias, Difficulties, Notes, Song } from './models.js'
import { MusicList } from './musicList.js'
import { AliasList } from './aliasList.js'

/** DF notes namedtuple → 领域 Notes（touch 缺省 0，total = 五项和） */
export function chartNotesToDomain(notes) {
  ...
  const brk = notes.brk ?? notes['break'] ?? 0
  return Notes({ tap, hold, slide, touch, brk, total: tap + hold + slide + touch + brk })
}
...
export function mergeMusicData({ divingFishList = null, lxnsList = null, statsMap = {} }) {
  ...
  for (const [sid, statList] of Object.entries(statsMap)) {
    ...
    song.difficulties.forEach((diff, i) => {
      if (statList[i]) diff.stats = statList[i]
    })
  }
